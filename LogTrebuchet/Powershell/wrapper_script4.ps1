#Updated version that checks for the size of what will be sent
...
#$outputFilePath = "windowsEvent.json"

$DceURI = "https://dctest-hxnt.westus-1.ingest.monitor.azure.com"
$DcrImmutableId = "dcr-df9f8e85fcdd4aa09f91957d63494311" #dcr-10565b4a76ce4484a0a32028759206bc    dcr-df9f8e85fcdd4aa09f91957d63494311
...
# If the data from a single post exceeds 30 MB, you should split the data into smaller sized chunks and send them concurrently."
...
foreach ($line in $jsonObjects) {
    ...
    $allowedProperties = Compare-Object -ReferenceObject $Properties -DifferenceObject $currentEventProperties.name -PassThru -ExcludeDifferent -IncludeEqual
    ...
    $arraySize = ([System.Text.Encoding]::UTF8.GetBytes(@(($chunkedData) | ConvertTo-Json))).Length
    $arrayProposedSize = ([System.Text.Encoding]::UTF8.GetBytes(@(($chunkedData + $message) | ConvertTo-Json))).Length


    # Check if adding the current message exceeds the maximum chunk size
    if ($arrayProposedSize -ge $maxChunkSize -or $line -eq $jsonObjects[-1]) {
        # Send the current chunk
        $headers2 = @{"Authorization" = "Bearer $bearerToken"; "Content-Type" = "application/json" };
        Write-Output $bearerToken
        $uri = "$DceURI/dataCollectionRules/$DcrImmutableId/streams/$table"+"?api-version=2023-01-01";
        $uploadResponse = Invoke-RestMethod -Uri $uri -Method "Post" -Body $message -Headers $headers2;
        $batchedMessage = $chunkedData | ConvertTo-Json -Compress -AsArray
        ...
        Write-Output "Content Length: $arraySize"
        Write-Output "---------------------"
